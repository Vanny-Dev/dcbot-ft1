...
client.on(Events.MessageCreate, msg => {
  ...
  if(!(msg.author.id === "1141665597126619206")){
    if(/[gG]ood\s?[mM]orning+/.test(input)){  
      msg.reply(`Good morning to you as well, hope you have a nice day! ${person}`) 
    }
  
  
    if(/[gG]ood\s?[aA]fternoon+/.test(input)){  
      msg.reply(`Good afternoon to you, have you eaten lunch already? ${person}`) 
    }
  
    if(/[gG]ood\s?[eE]vening+/.test(input)){  
      msg.reply(`Good evening to you as well, hope you are doing well until now ${person}`) 
    }
       ...
       let prefix = ">"
            ...
            if (text.length <= 1) {
                ...
                try{
                    ...
                    var program = {
                        ...
                        clientSecret: "4bc3d48db257f4cff8424ef812b3e2202e122d88a0f7cfba9848cbfa8d977ae7"
                      };
                      request({
        ...
        if (input.startsWith(prefix + "spamNGL")) {
            let datas = input.substring(9);
             if (datas.length < 2) {
              msg.reply("⚠️𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐔𝐬𝐞 𝐎𝐟 𝐂𝐨𝐦𝐦𝐚𝐧𝐝!\n💡𝐔𝐬𝐚𝐠𝐞: " + prefix + "spamNGL <username> <times> <text>")
            }
            else {
              try {
                 (async () => {
            ...
            const texts = datas.substring(datas.indexOf(' ') + count.length + 2);
                ...
                let deviceId = '', chars = '0123456789abcdefghijklmnopqrstuvwxyz-'
                for (i = 0; i < 36; i++) {
      ...
      //msg.react("❌", msg.author.id);
       ...
       msg.react("✅", msg.author.id);
          ...
          if (input.startsWith(prefix + "getQR")) {
            ...
            const image = new AttachmentBuilder('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=' + encodeURIComponent(data), {name: "img.png"})
            msg.reply({ files : [image] }) 
            } 
        }
        if (input.startsWith(prefix + "randomcat")) {
            
            const image = new AttachmentBuilder('https://cataas.com/cat', {name: "img.png"})
            msg.reply({ files : [image] })  
        }
